...
class Participants extends React.Component {
	...
	async componentDidMount() {
		...
		try {
			...
			let {docs:participants,...participantsPagination} = (await api.get('/participant')).data;
			
			if(participants.length>0) {
				
				let selectedParticipant = participants[0];
				let mode = 'view';
				this.setState({participants,selectedParticipant,participantsPagination,mode});
	...
	setParticipantPage = async (page) => {
        ...
        try {
            ...
            let {docs:participants,...participantsPagination} = (await api.get('/participant',{params})).data;
					...
					<Container className="my-4">
						<Row>
							<Col>
			        			...
			        			<p>Esta página foi desenvolvida para que você possa inserir novos participantes no sistema, visualizar suas informações ou removê-los. Os participantes com permissão de estudante não possuem acesso a essa área quando logados.</p>
			        			<hr/>
		        			</Col>
		        		</Row>
		        		<Row>
							...
							<Col lg={7}>
								{this.state.selectedParticipant.profile && <ImageForms image={this.state.selectedParticipant.profile.image} disabled/>}
								<ParticipantsForms
