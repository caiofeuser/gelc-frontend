...
export default function PublicProjectsList({ projects = [] }) {
        ...
        <Media.Body className="align-middle">
          <h5 className="mt-4">
            <Link
              ...
              to={`/projetos/${project._id}`}
            >
              {project.title}
            </Link>
          </h5>
          <small className="text-muted">
            {format(parseISO(project.createdAt), "dd 'de' MMMM, yyyy", {
              locale: pt,
            })}
          </small>
          <p>
            ({project.members.length}){" "}
            {project.members.length > 1 ? "Partipantes" : "Participante"}
            <br />
  ...
  return (
    <Row id="public-projects-list">
      <Col lg={6}>{projectsLeft.map(renderProjects)}</Col>
      <Col lg={6}>{projectsRight.map(renderProjects)}</Col>
    </Row>
  );
}
