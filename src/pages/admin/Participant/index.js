...
import {getAuthenticatedUserId,getAuthenticatedUserPermission} from '../../../resources/auth';
...
class Participant extends React.Component {
	...
	async componentDidMount() {
		...
		try {
			...
			let participant = (await api.get(`/participant/${getAuthenticatedUserId()}`)).data;
	...
	updateParticipant = async (form) => {
		...
		try {
			...
			participant = (await api.put(`/participant/${this.state.participant._id}`,participant)).data;
			this.setState({participant});
	...
	updateImage = async (form) => {
        ...
        try {
            ...
            if(form.get('file').name) {
            	...
            	if(this.state.participant.profile.image && this.state.participant.profile.image._id) {
            
                    await api.delete(`image/${this.state.participant.profile.image._id}`);
            
                }
            	
                image = (await api.post(`/image/participant/${this.state.participant._id}`,form)).data;

            }else if(form.get('alt')) {

                image = (await api.put(`/image/${this.state.participant.profile.image._id}`,{alt:form.get('alt')})).data;
			...
			<React.Fragment>
				...
				<main>
					<Container className="my-4">
						<Row>
							<Col>
								<h3 className="text-uppercase">{(this.state.participant.profile && this.state.participant.profile.name)?this.state.participant.profile.name: this.state.participant.email}</h3>
			        			<h4>Seja muito bem-vindo.</h4>
			        			{this.state.participant.profile?
			        				(<p>Nesta página você pode alterar suas informações dentro do sistema do GELC, sinta-se a vontade também para alterar seu avatar caso deseje.</p>):
			        				(<p>Parece que algumas informações sobre você ainda não foram cadastradas no nosso sistema, por favor preencha os campos em falta, assim os visitantes do site poderão conhecê-lo, também não esqueça de enviar sua melhor imagem :)</p>)
			        			}
			        			<hr/>
		        			</Col>
		        		</Row>
		        		<Row>
							...
							<Col lg={5}>
								<ImageForms image={this.state.participant.profile?this.state.participant.profile.image:undefined} onSubmit={this.updateImage} disabled={!this.state.participant.profile}/>
							</Col>
		        		</Row>
					</Container>
				</main>
