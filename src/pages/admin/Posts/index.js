...
class Posts extends React.Component {
    ...
    savePost = async (form) => {
        ...
        try {
            ...
            if(this.state.mode==='add') {
                ...
                let {data:selectedPost} = await api.post('/post/',{title,description,content,author});
                ...
                let {data:selectedPost} = await api.put(`/post/${this.state.selectedPost._id}`,post);
                this.setState({selectedPost,mode:'update'});
                this.setPostsPage();
    ...
    removePost = async (post) => {
        ...
        const confirmation = window.confirm(`Deseja realmente remover a publicação ${post.title}?`);
    ...
    acceptPost = async (post) => {
        ...
        try {
            ...
            let {data:selectedPost} = await api.put(`/post/${post._id}`,{accepted:true});
            this.setState({selectedPost,mode:'update'});
            this.setPostsPage();
    ...
    setPostsPage = async (page) => {
        ...
        try {
            ...
            let {docs:posts,...postsPagination} = (await api.get('/post',{params})).data;
            ...
            <React.Fragment>
                ...
                <main>
                	<Container className="my-4">
                        ...
                        <h4>Crie e edite pequenas postagens que podem ser acessadas em seu sistema.</h4>
                        <p>É recomendável que você trabalhe apenas com uma postagem por vez, não alterne de item sem salvar as alterações realizadas, isso pode acarretar em perdas de dados.</p>
                        <hr/>
                        <Row>
                            <Col lg={5}>
                                <PostsList 
                            ...
                            <Col lg={7}>
                                <Tabs defaultActiveKey="post" id="uncontrolled-tab">
                                    <Tab eventKey="post" title="Postagem">
                                        <PostsForms key={this.state.selectedPost._id} mode={this.state.mode} post={this.state.selectedPost} onSubmit={this.savePost}/>
                                    </Tab>
                                    <Tab eventKey="image" title="Imagem" disabled={(this.state.mode==='add')}>
                                        <ImageForms image={this.state.selectedPost.image} onSubmit={this.updateImage}/>
                                    </Tab>
                                </Tabs>
                            </Col>
                        </Row>
